...
import { Testimonial, testimonials, treckImages, trecks } from "@/lib/db/schema";
...
export default async function Home(): Promise<JSX.Element> {
  ...
  const allImages = await db.select().from(treckImages).orderBy(treckImages.order);
  const allTestimonials: Testimonial[] = await db.select().from(testimonials);

  const imagesByTreckId: Record<number, { imageUrl: string; order: number | null }[]> = {};
